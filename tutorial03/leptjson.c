...
/// @param n 数据大小（字节）
/// @return 数据起始地址
static void* lept_context_push(lept_context* c, size_t n) {
  void* ret;
  assert(n > 0);
  // 如果超容，进行扩容，扩大到原来的 1.5 倍
  if (c->top + n >= c->size) {
    if (c->size == 0) c->size = LEPT_PARSE_STACK_INIT_SIZE;
    while (c->top + n >= c->size) c->size += c->size >> 1;
...
/// @brief 从缓冲区中取出已有数据（已知数据大小）
/// @param c 上下文
/// @param n 数据大小（字节）
/// @return 数据起始地址
static void* lept_context_pop(lept_context* c, size_t n) {
  assert(c->top >= n);
  return c->stack + (c->top -= n);
}
  ...
  for (;;) {
    ...
    switch (ch) {
      ...
      case '\\':
        ...
        break;
      // 字符串结束标记，统计缓冲内容长度，取出内容作为解析结果
      case '\"':
        len = c->top - head;
        lept_set_string(v, (const char*)lept_context_pop(c, len), len);
        c->json = p;
        return LEPT_PARSE_OK;
      // JSON 字符串不允许出现空字符（JSON 中的空字符是以码点 \u0000 表示的，在本习题中不考虑）
      case '\0':
        c->top = head;
        return LEPT_PARSE_MISS_QUOTATION_MARK;
      default:
        // 处理一些非法字符（码点为 0x0 ~ 0x1f 的不可见字符）
...
// 清空 JSON 值并置为 null，如果存在动态内存释放也一并释放（比如字符串）
