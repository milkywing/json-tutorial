...
static int lept_parse_array(lept_context* c, lept_value* v) {
  ...
  for (;;) {
    ...
    if (*c->json == ',') {
      ...
      v->type = LEPT_ARRAY;
      v->u.a.size = size;
      size_t byte_size = size * sizeof(lept_value);
      memcpy(v->u.a.e = (lept_value*)malloc(byte_size), lept_context_pop(c, byte_size), byte_size);
      return LEPT_PARSE_OK;
    } else {
...
/// @brief 清空 JSON 值并置为 null，如果存在动态内存释放也一并释放（比如字符串/数组）
