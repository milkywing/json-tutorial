...
/// @brief 解析一段字符串（不包含结束标记），并通过入参返回字符串在缓冲区中的起始地址及其长度
/// @param c
/// @param str 字符串在缓冲区的起始地址
/// @param len 字符串长度
/// @return 解析错误码
...
static int lept_parse_object(lept_context* c, lept_value* v) {
  ...
  for (;;) {
    ...
    // 完成一个键值对的解析，并将解析结果写入缓冲区（移动）
    memcpy(lept_context_push(c, sizeof(lept_member)), &m, sizeof(lept_member));
    size++;
    // 上面已经将解析结果的所有权移交至缓冲区中，因此这里要重置下临时键，避免解析出错时重复释放
    m.k = NULL;
    lept_parse_whitespace(c);
    // 逗号或结束判断
    if (*c->json == ',') {
      ...
      v->type = LEPT_OBJECT;
      v->u.o.size = size;
      size_t byte_size = size * sizeof(lept_member);
      memcpy(v->u.o.m = (lept_member*)malloc(byte_size), lept_context_pop(c, byte_size), byte_size);
      return LEPT_PARSE_OK;
    } else {
  ...
  // 临时键值对可能遗留已经分配的键，需要释放
  if (m.k) free(m.k);
  // 解析失败时会走到这里，检查并释放当前缓冲区内的键值对
  while (size--) {
