  ...
  switch (lhs->type) {
    ...
    case LEPT_OBJECT:
      ...
      for (i = 0; i < lhs->u.o.size; i++) {
        ...
        lept_value* find = lept_find_object_value(rhs, m->k, m->klen);
...
// 在数组的 index 位置插入，并返回该位置的指针（不做边界检查，请确保 index 的合法性）
...
// 删除数组 index 位置起的 count 个元素（不做边界检查，请确保 index 的合法性）
...
void lept_move_member(lept_member* dst, lept_member* src) {
  assert(dst != NULL && src != NULL && src != dst);
  lept_free_member(dst);
  memcpy(dst, src, sizeof(lept_member));
  src->k = NULL;
  src->klen = 0;
  lept_init(&src->v);
}
...
// 删除对象 index 位置的键值对（不做边界检查，请确保 index 的合法性）
void lept_remove_object_value(lept_value* v, size_t index) {
  ...
  for (size_t i = index; i < v->u.o.size - 1; i++) {
    lept_move_member(v->u.o.m + i, v->u.o.m + i + 1);
  }
  v->u.o.size--;
}
